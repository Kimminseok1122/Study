...
function getCategory_List(e) {
	...
	if(obj.category_lv == 1 || obj.category_lv == 2) {
		...
		//console.log( $(e.target).date("set").group_id );
		//console.log( $(e.target).date("set").category_id );
	
		//$("input[name='prod_category']").val($(e.target).data("set").groud_id + $(e.target).data)
...
$.fn.category_set = function() {
	...
	$("input[name='category_key']").val(group_id + category_id );	 //name이 prod_category인 곳에 추가
}
//이전카테고리 삭제JS
